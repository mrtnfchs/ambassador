...
// HelmManagerOptions specifies options for creating the Helm manager
...
func NewHelmDownloader(options HelmManagerOptions) (HelmDownloader, error) {
...
func (lc *HelmDownloader) Download() error {
	...
	default:
		return fmt.Errorf("%w: u.Scheme", ErrUnknownHelmRepoScheme)
	}
...
// GetReleaseMgr returns a Helm release manager that works on a namespace
func (lc *HelmDownloader) GetReleaseMgr(namespace string, values map[string]string) (release.Manager, error) {
	// create a fake object for the Operator manager
	// the manager uses this instance as the owner of the Secret where the Helm chart info is stored.
	...
	return lc.GetReleaseMgrForInstance(&ins, values)
}
